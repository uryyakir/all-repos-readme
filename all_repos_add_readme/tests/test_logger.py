    ...
    with tempfile.TemporaryDirectory(
        ...
        for _ in range(constants.LOGFILES_ITERATION_COUNTER):
            setup_logger(
                ...
                verbose=False,
                log_file_name=LoggerConstants().tool_default_logfile_name,
        ...
        assert (
            ...
            == constants.LOGFILES_ITERATION_COUNTER
        )  # check total unique timestamps
    ...
    with tempfile.TemporaryDirectory(
        ...
        for _ in range(constants.LOGFILES_ITERATION_COUNTER):
            setup_logger(
                logger_name=LoggerConstants.TOOL_LOGGER_NAME,
                verbose=False,
                log_file_name=constants.CUSTOM_LOGFILE_NAME,
                logs_directory=tmpdir,
            )
            github_api.main(
                user_input=None,
...
def test_rotating_file_handler_backup_count_overflow(constants: TestConstants) -> None:
    with tempfile.TemporaryDirectory(
        ...
        assert (
            ...
            == LoggerConstants.ROTATING_FILE_HANDLER_BACKUP_COUNT + 1
        )  # check total files created, should be equal to total backups + 1 (original)
            ...
            list(
                map(
                    ...
                    [
                        ...
                        if file_ != constants.CUSTOM_LOGFILE_NAME
                    ],
                ),
            ),
        ) == list(
