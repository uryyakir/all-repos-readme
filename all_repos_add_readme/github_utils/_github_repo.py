...
class _Repo:
        ...
        self._used_languages: List[Dict[str, Union[float, str]]] = [
            {
                'language': language,
                'percentage': round(
                    100 * (total_lines / sum(repo.get_languages().values())),
                    2,
                ),
            }
    ...
    def used_languages(self) -> List[Dict[str, Union[float, str]]]:
        return sorted(
            self._used_languages, key=lambda dict_: dict_['percentage'], reverse=True,
        )
    ...
    def _inject_used_languages(self, formatted_readme: str) -> Soup:
        ...
        for i, language in enumerate(self.used_languages):
            ...
            current_language_tag.string = (
                f"{language['language']}: {str(language['percentage'])}%"
            )
    ...
    def generate_readme_string(self, user_input: Optional[str]) -> str:
        ...
        if user_input:
            ...
            with open(
                Constants.TOOL_README_TEMPLATE_PATH, encoding='utf-8',
            ) as readme_template_file:
            ...
            markdown_string = (
                with_formatted_languages.prettify()
                .replace('&gt;', '>')
                .replace('&lt:', '<')
            )
