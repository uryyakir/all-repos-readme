...
class GitHubAPI:
    def __init__(self, **kwargs: Any) -> None:
        ...
        self._commit_message = (
            kwargs[ToolArgumentNames.COMMIT_MESSAGE_ARGUMENT]
        ...
        if not github_repo.fork and not _should_ignore:
            try:
                ...
                if Constants.TOOL_NAME in readme_content:
                    ...
                    logger.debug(
                        f"found tool signature in {github_repo.full_name} README file - invoking README update"
                    )
                    raise RepoReadmeNeedsUpdate(readme_sha=readme_file.sha)

                else:
                    logger.debug(
                        f"found a README in {github_repo.full_name} that does not contain tool signature - skipping"
                    )
                    return None  # noop
    ...
    def _run_tool(
        self, github_repo: Repository, exception: UnknownObjectException
        ...
        logger.debug(
            f"finished generating README.md content for repo {github_repo.full_name}"
        )
        ...
        if isinstance(exception, RepoReadmeNeedsUpdate):
            ...
            if not self._dry_run:
                ...
                logger.debug(
                    f"finished updating README.md file in repo {github_repo.full_name}!"
                )
            ...
            if not self._dry_run:
                ...
                logger.debug(
                    f"finished creating README.md file in repo {github_repo.full_name}!"
                )
                return {github_repo.full_name: response["commit"].sha}
...
def _get_config_json(config_filepath: Optional[str]) -> Dict[str, str]:
    with open(
        config_filepath
        if config_filepath
        else GithubConstants.GITHUB_CONFIG_FILE.value,
        encoding="utf8",
    ) as config_file:
...
def main(
    ...
    **kwargs: Any,
    ...
    for github_repo in set(github.get_user().get_repos(affiliation="owner")):
        ...
        logger.info(
            f"tool run on repo {github_repo.full_name}: {LoggerColoring.GREEN.value}done\n{LoggerColoring.RESET_SEQ.value}{'*' * 10}"
        )
