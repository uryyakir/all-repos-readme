...
def test_getitem_does_not_exist(constants: TestConstants, git_config_handler_object: GitConfigHandler) -> None:
    with pytest.raises(NoSectionError):
        _ = git_config_handler_object[constants.TEST_PROPERTY_PATH]


@pytest.mark.run(order=2)
def test_delitem_does_not_exist(constants: TestConstants, git_config_handler_object: GitConfigHandler) -> None:
    assert not git_config_handler_object.__delitem__(constants.TEST_SECTION_NAME)


@pytest.mark.run(order=3)
def test_setitem(constants: TestConstants, git_config_handler_object: GitConfigHandler) -> None:
...
def test_delitem(constants: TestConstants, git_config_handler_object: GitConfigHandler) -> None:
