...
def test_tool_run(capsys: CaptureFixture[AnyStr], get_github_repository_object: Repository, constants: TestConstants) -> None:
...
def test_tool_dry_run(capsys: CaptureFixture[AnyStr], constants: TestConstants) -> None:
...
def test_tool_custom_commit_message(capsys: CaptureFixture[AnyStr], get_github_repository_object: Repository, constants: TestConstants) -> None:
    ...
    assert get_last_commit(get_github_repository_object).commit.message.startswith(constants.TEST_CUSTOM_COMMIT_MESSAGE) and \
        get_last_commit(get_github_repository_object).commit.message.endswith(Constants.TOOL_COMMIT_SIGNATURE)


def test_tool_custom_user_input(capsys: CaptureFixture[AnyStr], get_github_repository_object: Repository, constants: TestConstants) -> None:
...
def test_tool_create_missing_readme(capsys: CaptureFixture[AnyStr], get_github_repository_object: Repository, constants: TestConstants) -> None:
    get_github_repository_object.delete_file(path='README.md', message='delete README.md' + Constants.TOOL_COMMIT_SIGNATURE, sha=get_github_repository_object.get_readme().sha)
    exit_code = github_api.main(user_input=None, dry_run=False, _test_patterns_lst=constants.ONLY_TEST_AGAINST_REPO_FILTER)
    _ = _assert_expected_exit_code(capsys, exit_code, 1)
