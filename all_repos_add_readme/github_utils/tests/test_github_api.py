...
def test_tool_run(
    ...
    get_github_repository_object: Repository,
    constants: TestConstants,
) -> None:
    exit_code = github_api.main(
        user_input=None,
        dry_run=False,
        _test_patterns_lst=constants.ONLY_TEST_AGAINST_REPO_FILTER,
    )
    err_json = _assert_expected_exit_code(capsys, exit_code, 1)

    assert (
        get_last_commit(get_github_repository_object).sha
        == err_json[get_github_repository_object.full_name]
    )
...
def test_tool_custom_commit_message(
    capsys: CaptureFixture[AnyStr],
    get_github_repository_object: Repository,
    constants: TestConstants,
) -> None:
    exit_code = github_api.main(
        user_input=None,
        dry_run=False,
        commit_message=constants.TEST_CUSTOM_COMMIT_MESSAGE,
        _test_patterns_lst=constants.ONLY_TEST_AGAINST_REPO_FILTER,
    )
...
def test_tool_custom_user_input(
    capsys: CaptureFixture[AnyStr],
    get_github_repository_object: Repository,
    constants: TestConstants,
) -> None:
    exit_code = github_api.main(
        user_input=constants.TEST_USER_INPUT,
        dry_run=False,
        _test_patterns_lst=constants.ONLY_TEST_AGAINST_REPO_FILTER,
    )
    _ = _assert_expected_exit_code(capsys, exit_code, 1)
...
def test_tool_create_missing_readme(
    capsys: CaptureFixture[AnyStr],
    get_github_repository_object: Repository,
    constants: TestConstants,
) -> None:
